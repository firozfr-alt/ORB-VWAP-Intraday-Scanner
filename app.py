...
# ==========================================
st.set_page_config(
    page_title="OVERA System Trading Platform",
    page_icon="⚡",
    layout="wide",
...
# ==========================================
# HEADER & DATA MAPPINGS
# ==========================================
col_title, col_status = st.columns([3, 1])
with col_title:
    st.markdown("## ⚡ OVERA Master Trading Platform (Intraday & Swing Separate)")
with col_status:
    st.markdown("""
    <div style="text-align: right; padding-top: 10px;">
        <span style="background: rgba(16, 185, 129, 0.15); color: #10b981; border: 1px solid rgba(16, 185, 129, 0.3); padding: 6px 14px; border-radius: 20px; font-size: 12px; font-weight: 600;">
            <span class="live-dot"></span>OVERA Engine Active
        </span>
    </div>
    """, unsafe_allow_html=True)

st.markdown('<div class="subtitle-clean">Dedicated modular separation for OVERA Intraday and OVERA-S Liquid Swing routines[cite: 1].</div>', unsafe_allow_html=True)
st.markdown("---")

STOCK_TO_SECTOR = {
    "HDFCBANK.NS": "Nifty Bank", "ICICIBANK.NS": "Nifty Bank", "AXISBANK.NS": "Nifty Bank", "KOTAKBANK.NS": "Nifty Bank", "SBIN.NS": "Nifty Bank",
    "TCS.NS": "Nifty IT", "INFY.NS": "Nifty IT", "HCLTECH.NS": "Nifty IT", "TECHM.NS": "Nifty IT", "WIPRO.NS": "Nifty IT",
    "TATAMOTORS.NS": "Nifty Auto", "MARUTI.NS": "Nifty Auto", "M&M.NS": "Nifty Auto", "BAJAJ-AUTO.NS": "Nifty Auto",
    "TATASTEEL.NS": "Nifty Metal", "HINDALCO.NS": "Nifty Metal", "JSWSTEEL.NS": "Nifty Metal", "VEDL.NS": "Nifty Metal",
    "RELIANCE.NS": "Nifty 50 Core", "BAJFINANCE.NS": "Nifty Financial Services", "ITC.NS": "Nifty FMCG", "SUNPHARMA.NS": "Nifty Pharma"
}

SECTOR_PROXIES = {
    "Nifty Bank": "^NSEBANK", "Nifty IT": "^CNXIT", "Nifty Auto": "^CNXAUTO",
    "Nifty Metal": "^CNXMETAL", "Nifty Pharma": "^CNXPHARMA", "Nifty FMCG": "^CNXFMCG", "Nifty 50 Core": "^NSEI"
}
...
def get_sector_biases():
    ...
    for sec, ticker in SECTOR_PROXIES.items():
        try:
            ...
            if len(h) >= 2:
                biases[sec] = 1 if float(h["Close"].iloc[-1] - h["Close"].iloc[-2]) >= 0 else -1
            else:
                biases[sec] = 1
        except:
            biases[sec] = 1
...
# ==========================================
WATCHLIST_PRESETS = {
    "Nifty 50 Core": ["RELIANCE.NS", "TCS.NS", "HDFCBANK.NS", "INFY.NS", "ICICIBANK.NS", "SBIN.NS", "TATAMOTORS.NS", "TATASTEEL.NS"],
    "Banking & Financials": ["HDFCBANK.NS", "ICICIBANK.NS", "SBIN.NS", "AXISBANK.NS", "KOTAKBANK.NS"],
    "High-Beta / F&O Momentum": ["TATAMOTORS.NS", "BAJFINANCE.NS", "ADANIENT.NS", "HINDALCO.NS", "VEDL.NS"]
}
...
st.sidebar.markdown("---")
st.sidebar.subheader("Intraday Tuning (OVERA)")
rvol_mult = st.sidebar.slider("Min Intraday RVOL", 1.0, 3.0, 1.2, 0.1)
min_adx = st.sidebar.slider("Min ADX Threshold", 10, 30, 18, 1)

st.sidebar.markdown("---")
st.sidebar.subheader("Swing Tuning (OVERA-S)")
min_swing_rvol = st.sidebar.slider("Min Swing RVOL (20d)", 1.0, 3.0, 1.5, 0.1)
...
def analyze_overa_intraday(ticker_symbol: str, timeframe: str):
    try:
        ...
        if len(today_df) < 3: return {"error": "Waiting for bars."}
        ...
        ema20 = float(ta.trend.ema_indicator(today_df["Close"], window=20).iloc[-1])
        ema50 = float(ta.trend.ema_indicator(today_df["Close"], window=50).iloc[-1])
        ...
        long_cond = (ltp > or_high) and (ltp > vwap_latest) and (ema20 > ema50) and (adx_val >= min_adx) and (50 <= rsi_val <= 70) and (rvol >= rvol_mult) and (s_bias == 1)
        short_cond = (ltp < or_low) and (ltp < vwap_latest) and (ema20 < ema50) and (adx_val >= min_adx) and (30 <= rsi_val <= 50) and (rvol >= rvol_mult) and (s_bias == -1)

        return {
            "Symbol": ticker_symbol.replace(".NS", ""), "LTP": round(ltp, 2),
            "VWAP": round(vwap_latest, 2), "RSI": round(rsi_val, 1), "ADX": round(adx_val, 1),
            "RVOL": round(rvol, 2), "LongSetup": long_cond, "ShortSetup": short_cond, "Sector": sec_name
        }
    except: return None

def analyze_overa_swing(ticker_symbol: str):
    try:
        stock = yf.Ticker(ticker_symbol)
        ...
        # OVERA-S Liquid Rules: Close > 200 SMA, 20-day high breakout, EMA20 > EMA50, ADX > 18, RSI 55-75, RVOL >= min_swing_rvol[cite: 1]
        is_setup = (
            (ltp > sma_200) and
            (ltp > highest_20) and
            (ema_20 > ema_50) and
            (adx_val >= 18) and
            (55 <= rsi_val <= 75) and
            (rvol_20 >= min_swing_rvol)
        )

        score = round((rsi_val / 50) * rvol_20, 2)

        return {
            "Symbol": ticker_symbol.replace(".NS", ""), "LTP": round(ltp, 2),
            "Score": score, "RSI": round(rsi_val, 1), "ADX": round(adx_val, 1),
            "RVOL_20": round(rvol_20, 2), "ATR": round(atr, 2), "Is_Setup": is_setup
        }
    except: return None

# ==========================================
# DASHBOARD TABS (SEPARATED OVERA INTRADAY & SWING)
# ==========================================
tab_intraday, tab_swing, tab_sector, tab_inst = st.tabs([
    "⚡ OVERA Intraday (15m / 5m)", 
...
with tab_intraday:
    ...
    <div class="strategy-box-1">
        ...
        <p class="strategy-desc">Same-day entry/exit using Opening Range, VWAP, EMA20/50 stack, ADX > 18, RSI bands, and volume conviction[cite: 1].</p>
    </div>
    """, unsafe_allow_html=True)

    timeframe_choice = st.radio("Select Intraday Timeframe", ["15-Min", "5-Min"], horizontal=True)
    tf_code = "15m" if timeframe_choice == "15-Min" else "5m"

    if st.button("🚀 Run OVERA Intraday Scan", type="primary", key="btn_intraday"):
        longs, shorts = [], []
        bar = st.progress(0)
        for i, sym in enumerate(symbols_to_scan):
            bar.progress((i + 1) / len(symbols_to_scan))
            res = analyze_overa_intraday(sym, timeframe=tf_code)
            if not res or "error" in res: continue
            if res["LongSetup"]:
                longs.append({"Stock": res["Symbol"], "Sector": res["Sector"], "LTP (₹)": res["LTP"], "VWAP": res["VWAP"], "RSI": res["RSI"], "ADX": res["ADX"], "RVOL": f"{res['RVOL']}x"})
            elif res["ShortSetup"]:
                shorts.append({"Stock": res["Symbol"], "Sector": res["Sector"], "LTP (₹)": res["LTP"], "VWAP": res["VWAP"], "RSI": res["RSI"], "ADX": res["ADX"], "RVOL": f"{res['RVOL']}x"})
        bar.empty()
        c1, c2 = st.columns(2)
        with c1:
            st.markdown("#### 🟢 OVERA Intraday Longs")
            st.dataframe(pd.DataFrame(longs) if longs else pd.DataFrame([{"Status": f"No strict OVERA {tf_code} long setups"}]), use_container_width=True)
        with c2:
            st.markdown("#### 🔴 OVERA Intraday Shorts")
            st.dataframe(pd.DataFrame(shorts) if shorts else pd.DataFrame([{"Status": f"No strict OVERA {tf_code} short setups"}]), use_container_width=True)

with tab_swing:
    st.markdown("""
    <div class="strategy-box-2">
        <h3 class="strategy-title">📈 OVERA-S Liquid Swing Scanner (3-10 Day Holds)</h3>
        <p class="strategy-desc">Evaluates 200-SMA quality gate, 20-day high breakouts, EMA stack, ADX, RSI 55-75, and volume surge[cite: 1].</p>
    </div>
    """, unsafe_allow_html=True)
    ...
    if st.button("🚀 Run OVERA-S Swing Scan", type="primary", key="btn_swing"):
        ...
        for i, sym in enumerate(symbols_to_scan):
            bar.progress((i + 1) / len(symbols_to_scan))
            s = analyze_overa_swing(sym)
            if s and s["Is_Setup"]:
                ltp, atr = s["LTP"], s["ATR"]
                candidates.append({
                    "Stock": s["Symbol"], "LTP (₹)": ltp, "Score": s["Score"],
                    "RSI": s["RSI"], "ADX": s["ADX"], "RVOL (20d)": f"{s['RVOL_20']}x", 
                    "Stop-Loss": round(ltp - (2.0 * atr), 2), "Target": round(ltp + (3.0 * atr), 2)
                })
        bar.empty()
        if candidates:
            st.markdown("#### 🚀 Qualified OVERA-S Swing Setups")
...
with tab_sector:
    ...
    with col_buy:
        st.markdown("#### 🟢 Top 5 Buy Stocks (Leading Sectors)")
    ...
    with col_sell:
        st.markdown("#### 🔴 Top 5 Short Stocks (Lagging Sectors)")
        st.dataframe(pd.DataFrame([
...
with tab_inst:
    ...
    <div class="strategy-box-5">
        <h3 class="strategy-title">Multi-Cap Institutional Flow Radar</h3>
        <p class="strategy-desc">Monitors institutional accumulation and distribution.</p>
    </div>
    """, unsafe_allow_html=True)
    st.dataframe(pd.DataFrame([
